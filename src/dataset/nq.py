...
def get_nq():
    data: list[Item] = []
    dataset_path = "download/data/cbxgss/rag/nq/test.jsonl"
    with open(dataset_path,"r",encoding="utf-8") as f:
        for line in f:
            item = json.loads(line)
            id, question, golden_answers = item["id"], item["question"], item["golden_answers"]
            metadata: dict = item
            metadata.pop("id")
            metadata.pop("question")
            metadata.pop("golden_answers")
            data.append(Item(id=id, question=question, golden_answers=golden_answers, metadata=metadata))
    dataset = Dataset(name="nq", data=data)
    reanswer = """### 背景说明
