...
# path for monitor
EXPERT_MODEL_PATH = {
    "t5": "gaussalgo/T5-LM-Large_Canard-HotpotQA-rephrase",     # 这篇论文中效果最好的模型
}
...
```json 
{{
    "reason": "your reasoning steps",
    "answer": "your final answer in ONE OR FEW WORDS"
}}
```
...
```json
{{
    "reason": "The husband of Catherine of Pomerania, Countess Palatine of Neumarkt is John, Count Palatine of Neumarkt. The father of John, Count Palatine of Neumarkt is Rupert III of the Palatinate. So the final answer is: Rupert III of the Palatinate.",
...
QA_response_format = {
    ...
    "json_schema": {
        "name": "ReasonAnswer",
        "schema": SchemaQA.model_json_schema()
    }
}
...
```json
{{
    "judgement": "the error type(or correct)",
    "feedback": "some short reminders you would like to give"
}}
```
"""
...
CHECK_response_format = {
    "type": "json_schema",
    "json_schema": {
        "name": "JudgementFeedback",
        "schema": SchemaCheck.model_json_schema()
    }
}
